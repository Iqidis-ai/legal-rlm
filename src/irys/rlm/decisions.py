...
# =============================================================================
# EXTERNAL CITATION SELECTION (LITE model)
# =============================================================================

async def select_external_citations(
    answer: str,
    case_law: list[dict],
    web: list[dict],
    client: GeminiClient,
    ...
    """Select which external sources are actually referenced in the answer.

    Uses LITE model — this is a simple list-selection call, not deep reasoning.
    On any failure falls back to first `fallback_n` from each list.

    Args:
        answer:     The synthesized answer text.
        case_law:   All staged case law dicts (keys: case_name, citation, snippet).
        web:        All staged web dicts (keys: title, content, url).
        client:     GeminiClient instance.
        fallback_n: How many to take from each list when selection fails.

    Returns:
        (selected_case_law_indices, selected_web_indices) — both 0-based.
    """
    if not case_law and not web:
        return [], []

    def _fallback() -> tuple[list[int], list[int]]:
        return list(range(min(fallback_n, len(case_law)))), list(range(min(fallback_n, len(web))))

    try:
        # Build human-readable lists for the prompt
        case_law_lines = "\n".join(
            f"[{i}] {c.get('case_name', 'Unknown')} | {c.get('citation', '')} | {(c.get('snippet') or c.get('opinion_text') or '')[:120]}"
            for i, c in enumerate(case_law)
        )
        web_lines = "\n".join(
            f"[{i}] {r.get('title', 'Unknown')} | {(r.get('content') or '')[:120]}"
            for i, r in enumerate(web)
        )

        prompt = prompts.P_SELECT_EXTERNAL_CITATIONS.format(
            answer=answer[:100000],
            case_law_list=case_law_lines or "(none)",
            web_list=web_lines or "(none)",
        )

        start_time = time.time()
        response = await client.complete(prompt, tier=ModelTier.LITE)
        _log_llm_result("select_external_citations", response, time.time() - start_time)

        parsed = parse_json_safe(response)
        if not parsed:
            logger.warning("select_external_citations: JSON parse failed, using fallback")
            return _fallback()

        cl_indices = [
            i for i in parsed.get("case_law_indices", [])
            if isinstance(i, int) and 0 <= i < len(case_law)
        ]
        web_indices = [
            i for i in parsed.get("web_indices", [])
            if isinstance(i, int) and 0 <= i < len(web)
        ]

        # Safety net: if LLM returned nothing for either list, keep fallback
        if not cl_indices and case_law:
            logger.info("select_external_citations: no case law selected by LLM, using fallback")
            cl_indices = list(range(min(fallback_n, len(case_law))))
        if not web_indices and web:
            logger.info("select_external_citations: no web selected by LLM, using fallback")
            web_indices = list(range(min(fallback_n, len(web))))

        logger.info(f"select_external_citations: {len(cl_indices)} case law, {len(web_indices)} web selected")
        return cl_indices, web_indices

    except Exception as e:
        logger.warning(f"select_external_citations failed ({e}), using fallback")
        return _fallback()
