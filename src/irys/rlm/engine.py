...
class RLMEngine:
    ...
    def _enforce_advocacy_gate(self, synthesis_output: str) -> Optional[str]:
        ...
        # Content-based check: our marker is only present when we already injected it.
        _MARKER = "Source Calibration Advisory"
        if _MARKER in synthesis_output:
            return None  # already present — gate satisfied

        # Build issue index for human-readable names.
        issue_index: dict = {}
        try:
        ...
        lines = [
            ...
            "",
            "The conclusions below rely exclusively on advocacy-authored material "
            "(pleadings, briefs, demand letters). They must NOT be presented as "
            "established facts without independent corroboration:",
            "",
        ]
